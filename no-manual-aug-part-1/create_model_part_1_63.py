...
orig_epochs = loadtxt('final_input_data_with_labels_1_63.csv', delimiter=',', skiprows=1)
print(orig_epochs.shape)

# scale the data with robust scaler
my_epochs = rScaler.fit_transform(orig_epochs[0:92, 0:3969])  # noise
...
# Create the model
model=Sequential()
model.add(Conv1D(filters=20, kernel_size=6, kernel_regularizer=L2(0.001), bias_regularizer=L2(0.001), activity_regularizer = L2(0.001), kernel_constraint=min_max_norm(min_value=-0.25, max_value=0.25), padding='valid', activation='relu', strides=1, input_shape=(63, 63)))
model.add(Conv1D(filters=20, kernel_size=5, kernel_regularizer=L2(0.001), bias_regularizer=L2(0.001), activity_regularizer = L2(0.001), kernel_constraint=min_max_norm(min_value=-0.25, max_value=0.25), padding='valid', activation='relu', strides=1))
model.add((AveragePooling1D()))
model.add(Conv1D(filters=20, kernel_size=4, kernel_regularizer=L2(0.001), bias_regularizer=L2(0.001), activity_regularizer = L2(0.001), kernel_constraint=min_max_norm(min_value=-0.25, max_value=0.25), padding='valid', activation='relu', strides=1))
model.add(GlobalMaxPooling1D())
model.add(Dense(20, kernel_regularizer=L2(0.001), bias_regularizer=L2(0.001), activity_regularizer = L2(0.001), kernel_constraint=min_max_norm(min_value=-0.25, max_value=0.25), activation='relu'))
...
hist = model.fit(input, Y_train, batch_size=24, epochs=400, verbose=1, validation_data=(testinput, Y_test), steps_per_epoch=None, callbacks=[es, mc])
